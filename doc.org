...
oai-parse-org-header -> oai-request-prepare -> oai-restapi--url-request
...
* oai-timers.el: Progress Reporter and counting requests [2026-05-12 Tue]
modeline set with: ~oai-timers--oai-update-mode-line~ in:
  - ~(length (oai-timers--get-all-keys))~ - get count of all requests
...
- ~oai-timers--progress-reporter-run~ - run timer that just make reporter visible every second
- ~oai-timers--stop-global-progress-reporter~

New request run: ~oai-timers--progress-reporter-run~
- set modeline, count

“Now, we dont show” comment used for disabling ouput of current request in mideline
