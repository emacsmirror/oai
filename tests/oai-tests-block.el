...
(ert-deftest oai-tests-block--let-params-all-from-info-test1 ()
  ...
  (with-temp-buffer
    ...
    (let* ((test-block "#+begin_ai :stream t :stream1 t :sys \"A helpful LLM.\" :stream2 :max-tokens 50 :max-tokens2 :model \"gpt-3.5-turbo\" :model1 :model2 t :model3 :temperature 0.7\n#+end_ai\n")
      ...
      (let ((stream (oai-block--get-val info :stream "STREAM" nil 'string))
            (stream1 (oai-block--get-val info :stream1 "STREAM1" t 'bool))
            (stream2 (oai-block--get-val info :stream2 "STREAM2" 0 'number))
            (stream3 (oai-block--get-val info :stream3 "STREAM3" 1 'number))
            (sys (oai-block--get-val info :sys "SYS" nil 'string))
            (max-tokens (oai-block--get-val info :max-tokens "MAX-TOKENS" nil 'integer))
            (max-tokens2 (oai-block--get-val info :max-tokens2 "MAX-TOKENS2" 10 'integer))
            (model (oai-block--get-val info :model "MODEL" nil 'string))
            (model1 (oai-block--get-val info :model1 "MODEL1" nil 'string))
            (model2 (oai-block--get-val info :model2 "MODEL2" 10 'number))
            (model4 (oai-block--get-val info :model4 "MODEL4" nil 'number))
            (model3 (oai-block--get-val info :model3 "MODEL3" nil 'string))
            (temperature (oai-block--get-val info :temperature "TEMPERATURE" nil 'float))
            (unknown (oai-block--get-val info :unknown nil "s" )))
                                         ;; (print (list "model3" (type-of model3) model3 )))))
...
(ert-deftest oai-tests-block--let-params-all-from-info-test2 ()
  (cl-letf (((symbol-function 'org-entry-get-with-inheritance)
    ...
    (let ((info '((:model)
      ...
      (let ((model   (oai-block--get-val info :model   nil   nil 'string))
            (model1  (oai-block--get-val info :model1  nil  nil 'string))
            (model2  (oai-block--get-val info :model2  nil  nil 'string))
            (stream  (oai-block--get-val info :stream  nil  nil 'bool))
            (stream1 (oai-block--get-val info :stream1 nil nil 'bool))
            (stream2 (oai-block--get-val info :stream2 nil nil 'bool))
