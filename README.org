...
oai.el
 ...
 ├─ oai-restapi.el (→ oai-block.el, oai-block-msgs.el, oai-async1.el, oai-timers.el)
 └─ oai-prompt.el (→ oai-block.el, oai-block-msgs.el, oai-block-tags.el, oai-restapi.el, oai-async1.el, oai-timers.el)
#+end_src
* Supported services *
- remote: any server that provides an OpenAI-compliant endpoint to LLM model or AI agent.
...
** How images sent?
We do it in two steps, first we replace image link or tag with ~@image_jpeg:/path.jpg~ in ~oai-block-tags-replace~ and in ~oai-block-tags--compose-block-for-path-full~ as any other link, then ~oai-restapi--payload~ we encode image to base64 and split text around link and replace string in :context to a vector.
